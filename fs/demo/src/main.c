...
static int demo_readdir(const char* path, void* buf, fuse_fill_dir_t filler, off_t offset, struct fuse_file_info* fi)
{
    ...
    unsigned int off= 500 * super.sz_blks;
    ddriver_seek(super.driver_fd, off, 0);
    /* TODO: 调用ddriver_read读出一个磁盘块到内存，512B */
    char buff[512] = {0};
    ddriver_read(super.driver_fd, buff, super.sz_io);
    /* TODO: 使用memcpy拷贝上述512B的前sizeof(demo_dentry)字节构建一个demo_dentry结构 */
    struct demo_dentry tmp = {};
    memcpy(&tmp, buff, sizeof(struct demo_dentry));
    /* TODO: 填充filename */
    memcpy(filename, tmp.fname, sizeof(tmp.fname));
...
static int demo_getattr(const char* path, struct stat *stbuf)
{
    if(strcmp(path, "/") == 0)
        ...
        stbuf->st_mode = S_IFREG/* TODO: 显示为普通文件 */;            // 该文件显示为普通文件
    return 0;
}
